...
const CONNECTOR_VERSION =
  '2025-12-08 - edit_file direct replace-all all-file-types + diff-fallback v5';
...
function computeSessionDiffText(origPath, currentPath) {
  // Fallback: simple Node-based diff if WSL/`diff` is unavailable or fails.
  const fallback = () => {
    return Promise.all([
      fs.promises.readFile(origPath),
      fs.promises.readFile(currentPath),
    ])
      .then(([origBuf, currBuf]) => {
        if (!origBuf || !currBuf) {
          return null;
        }
        ...
        const currText = currBuf.toString('utf8');
        return [
          `--- ORIGINAL: ${origPath}`,
          origText,
          `--- CURRENT: ${currentPath}`,
      ...
      .catch((e) => {
        console.error('edit_file session diff fallback failure:', e);
        return null;
      });
  };
  ...
  return new Promise((resolve) => {
    ...
    try {
      const origWsl = windowsPathToWsl(origPath);
      const currWsl = windowsPathToWsl(currentPath);
      if (!origWsl || !currWsl) {
        // Cannot map to WSL paths; fall back to simple diff.
      ...
      const args = [
        'bash',
        '-lc',
        `diff -u '${origWsl}' '${currWsl}' || true`,
      ];
      execFile(cmd, args, { encoding: 'utf8' }, (err, stdout) => {
        if (err && !stdout) {
          console.error('edit_file session diff exec error:', err);
          fallback().then(resolve);
          return;
        }
        ...
        // If diff produced no output (files identical) or only whitespace,
        // use the fallback to double-check and possibly show a simple view.
        fallback().then(resolve);
      });
    } catch (e) {
      console.error('edit_file session diff failure:', e);
      fallback().then(resolve);
    }
  });
}
      ...
      try {
        ...
        if (buf && buf.length > 0) {
          ...
          if (sessionHash && !sessionHashArg) {
            try {
              sessionDir = path.join(EDIT_FILE_TRACKING_ROOT, sessionHash);
