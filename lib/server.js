...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  // Inner CMD command (what we want CMD to execute non-blocking):
  //   cmd /c ""C:\\...\\Merge.exe" "E:\\...left.md" "E:\\...right.md""
  // We pass this as a single argument to cmd.exe via Start-Process, so
  // CMD handles all the quoting and spacing exactly as if we had typed
  // it directly at a CMD prompt.
  const cmdInner = `""${araxisExe}" "${p1}" "${p2}""`;

  // Inner PowerShell command (with ARAXIS_SAFE marker for the guard):
  //   #ARAXIS_SAFE
  //   Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', '""Merge.exe" "left" "right""'
  //
  // Start-Process is non-blocking (no -Wait), so the MCP tool call
  // returns immediately while CMD + Araxis run in the background.
  const psCommand =
    "#ARAXIS_SAFE\n" +
    `Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', '${cmdInner}'`;

  // Wrap for PowerShell -Command. We use -NoProfile to keep startup fast
  // and avoid user profile side effects.
  return (
