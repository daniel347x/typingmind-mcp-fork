...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  const araxisExe =
    'C:\\Users\\danie\\AppData\\Local\\Apps\\Araxis\\Araxis Merge\\Merge.exe';
  ...
  // Nuclear option for argument passing: Base64 EncodedCommand.
  // This bypasses all shell quoting hell (cmd.exe, run_command parsing, etc.)
  // and ensures PowerShell receives the exact strings we intend.
  //
  // Inner PowerShell command:
  // Start-Process -FilePath '...' -ArgumentList 'p1', 'p2'
  
  const psCommand = `Start-Process -FilePath '${araxisExe}' -ArgumentList '${p1}', '${p2}'`;
...
function isSafePowerShellCommand(rawCommand) {
  ...
  let cmd = rawCommand.toLowerCase();

  // Decode EncodedCommand if present to check its contents
  // We look for -EncodedCommand (or -e, -en, -enc, etc.) followed by base64
  // We support the various parameter name abbreviations PowerShell accepts.
  const encodedMatch = cmd.match(/-(?:e|en|enc|enco|encod|encode|encoded|encodedc|encodedco|encodedcom|encodedcomm|encodedcomma|encodedcomman|encodedcommand)\s+(?:["']?)([a-zA-Z0-9+/=]+)(?:["']?)/i);
  
  if (encodedMatch) {
    try {
      const base64 = encodedMatch[1];
      // PowerShell EncodedCommand is UTF-16LE
      const decoded = Buffer.from(base64, 'base64').toString('utf16le').toLowerCase();
      // console.log('Decoded PowerShell command:', decoded);
      // Use the decoded command for safety checks instead of the wrapper
      cmd = decoded;
      
      // Log successful decoding for debug visibility
      try {
        const fs = require('fs');
        const RUN_COMMAND_CLEANSE_LOG = 'E:\\__daniel347x\\__Obsidian\\__Inking into Mind\\--TypingMind\\Projects - All\\Projects - Individual\\TODO\\temp\\run_command_cleanse.log';
        fs.appendFile(
          RUN_COMMAND_CLEANSE_LOG,
          JSON.stringify({
            timestamp: new Date().toISOString(),
            marker: 'run_command_cleansing_v1',
            kind: 'powershell-encoded-decode-success',
            original: rawCommand.substring(0, 50) + '...',
            decoded: cmd
          }) + '\n',
          () => {}
        );
      ...
      console.error('Failed to decode PowerShell EncodedCommand for safety check:', e);
      return false; // Fail closed if we can't decode
    }
  }
  ...
  return hasSnapnax || hasObsidian || hasAraxis || hasGitPush || hasCopyItem || hasWindSurf;
}
