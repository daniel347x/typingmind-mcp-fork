...
const CONNECTOR_VERSION =
  '2025-12-08 - edit_file direct replace-all all-file-types + python-diff + timestamped-sessions + tags + failure-summaries + success-diff v10';
...
async function runStructuredFileWithStage2Guard(clientEntry, context) {
  ...
  // direct replace-all semantics and backup/rollback apply consistently. The
  // structural validators are only activated for known structured kinds
  ...
  try {
    if (useDirectReplaceAll) {
      ...
      if (
          ...
          content: [
            {
              type: 'text',
              text:
                'edit_file direct replace-all requires a single edit with string oldText and newText.',
            },
          ],
        };
          ...
          content: [
            {
              type: 'text',
              text:
                'edit_file direct replace-all: empty oldText is not allowed.',
            },
          ],
        };
      } else {
        ...
        console.log(
          '[MCP CONNECTOR] edit_file direct replace-all on %s (occurrences=%d, oldLen=%d, newLen=%d)',
          filePath,
          occurrences,
          edit.oldText.length,
          edit.newText.length,
        ...
        if (occurrences === 0) {
          toolResult = {
            ...
            content: [
              {
                type: 'text',
                text: `edit_file direct replace-all: 0 occurrences of oldText found in ${filePath}.`,
              },
            ],
            directReplaceAll: {
              occurrences,
          ...
          const updatedText = parts.join(edit.newText);
          ...
          toolResult = {
            ...
            content: [
              {
                type: 'text',
                text: `edit_file direct replace-all applied ${occurrences} occurrence${
                  occurrences === 1 ? '' : 's'
...
// Global run_command lock for generic Git commands. We serialize all detected
// git-related run_command calls through a single queue with a 500ms guard
// interval, mirroring the QUILL/JEWEL semantics. This ensures that multiple
// git add/commit/push sequences never overlap in time.
async function runGitRunCommandWithLock(clientEntry, context) {
  const key = 'git-global';
  const previous = GIT_RUN_LOCKS.get(key) || Promise.resolve();
  const alreadyQueued = GIT_RUN_LOCKS.has(key);
  const callId = context.callId || 'unknown';

  try {
    logRunCommandCleanse({
      stage: 'git-run-lock-queue',
      callId,
      toolName: context.toolName,
      command:
        (context && context.toolArgs && context.toolArgs.command) || '(none)',
    ...
    console.error('git run-command lock queue log failure:', e);
  }

  const chained = previous
    .catch(() => {
      // Swallow errors from prior Git operations so they don't break the chain.
    })
    .then(async () => {
      try {
        try {
          logRunCommandCleanse({
            stage: 'git-run-lock-run',
            callId,
            toolName: context.toolName,
            command:
              (context && context.toolArgs && context.toolArgs.command) ||
          ...
          console.error('git run-command lock run log failure:', e);
        }

        // Match the same 500ms guard interval used for edit_file / QUILL / JEWEL.
        await new Promise((resolve) => setTimeout(resolve, 500));
    ...
    .finally(() => {
      const current = GIT_RUN_LOCKS.get(key);
      if (current === chained) {
        GIT_RUN_LOCKS.delete(key);
      }
      try {
        logRunCommandCleanse({
          stage: 'git-run-lock-complete',
          callId,
          toolName: context.toolName,
          command:
            (context && context.toolArgs && context.toolArgs.command) ||
        ...
        console.error('git run-command lock complete log failure:', e);
      }
    });

  GIT_RUN_LOCKS.set(key, chained);
  return chained;
}
...
/**
 * Detect whether a tool call is a git-related run_command operation.
 * We use a simple heuristic: any run_command whose command string
 * contains the substring "git" (case-insensitive). This intentionally
 * allows false positives to ensure ALL git operations are serialized.
 */
function isGitRelatedOperation(toolName, toolArgs) {
  if (toolName !== 'run_command' || !toolArgs || typeof toolArgs.command !== 'string') {
    return false;
  }
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (context.toolName === 'edit_file') {
    ...
    for (let i = 0; i < cleanedArgs.edits.length; i += 1) {
      ...
      for (;;) {
        ...
        if (result && result.isError) {
          let successUntilPath = sessionSuccessPath || null;
          let restoreError = null;
          ...
          if (origBackupPath && effectivePath) {
            try {
              await fs.promises.copyFile(origBackupPath, effectivePath);
            } catch (e) {
              restoreError = e;
              console.error(
                'edit_file multi-edit: failed to restore original from orig backup:',
                origBackupPath,
                '->',
                effectivePath,
                e,
          ...
          // Write remaining-edits JSON for easier resume/fix-and-retry
          let remainingEditsPath = sessionEditsPath || null;
          try {
            const remainingSpec = {
              path: effectivePath || basePath,
              failed_at_index: i,
              passes_attempted: passes + 1,
              edits: cleanedArgs.edits.slice(i).map((e) =>
                e && typeof e === 'object' ? { ...e } : e,
              ),
            };
            ...
            if (remainingEditsPath) {
              await fs.promises.writeFile(
                remainingEditsPath,
                JSON.stringify(remainingSpec, null, 2),
                'utf8',
              );
            }
          } catch (e) {
            console.error(
              'edit_file multi-edit: failed to write remaining-edits spec:',
              e,
            );
          }
          ...
          if (remainingEditsPath) {
            baseContent.push({
              ...
              text:
                '\n---\n' +
                '💡 Multi-edit helper (MCP connector):\n' +
                `Remaining edits were saved to: ${remainingEditsPath}\n\n` +
                'You can QUILLSTRIKE that JSON file to refine oldText/newText and QUILLMORPH to try the pipeline again.\n',
            });

            try {
              const failedEdit = cleanedArgs.edits[i] || null;
              if (
                failedEdit &&
                (typeof failedEdit.oldText === 'string' ||
                  typeof failedEdit.newText === 'string')
              ) {
                const oldPreview = clipForLog(
                  typeof failedEdit.oldText === 'string'
                    ? failedEdit.oldText
                    : '',
                  128,
                );
                const newPreview = clipForLog(
                  typeof failedEdit.newText === 'string'
                    ? failedEdit.newText
                    : '',
                  128,
                ...
                baseContent.push({
                  ...
                  text:
                    '\n---\n' +
                    `❌ Failed edit at index ${i} (clipped to 128 chars):\n` +
                    `oldText:\n${oldPreview}\n\n` +
                    `newText:\n${newPreview}\n`,
                });
              }

              const remainingRaw = await fs.promises.readFile(
                remainingEditsPath,
                'utf8',
              );
              let remainingSummary = remainingRaw;
              try {
                const parsed = JSON.parse(remainingRaw);
                if (parsed && Array.isArray(parsed.edits)) {
                  const clippedEdits = parsed.edits.map((e, idx2) => {
                    const o =
                      typeof e.oldText === 'string'
                        ? clipForLog(e.oldText, 128)
                        : e.oldText;
                    const n =
                      typeof e.newText === 'string'
                        ? clipForLog(e.newText, 128)
                        : e.newText;
                    return { ...e, oldText: o, newText: n };
                  });
                  const summaryObj = {
                    path: parsed.path,
                    failed_at_index: parsed.failed_at_index,
                    passes_attempted: parsed.passes_attempted,
                    edits: clippedEdits,
                  };
                  remainingSummary = JSON.stringify(summaryObj, null, 2);
                }
              } catch (e) {
              ...
              baseContent.push({
                ...
                text:
                  '\n---\n' +
                  '📝 edits_remaining.json (clipped oldText/newText):\n\n' +
                  remainingSummary +
                  '\n',
              });
            } catch (e) {
              console.error(
                'edit_file: failed to summarize remaining-edits JSON:',
                e,
              );
            }
          }
          ...
          try {
            ...
            const currentForDiff = successUntilPath || effectivePath;
            const diffText = await computeSessionDiffText(
              origForDiff,
              currentForDiff,
            );
            if (diffText) {
            ...
            console.error('edit_file session diff (failure path) error:', e);
          }
          ...
          return {
            ...
            multiEdit: {
              failed_edit_index: i,
              total_edits: cleanedArgs.edits.length,
              passes_attempted: passes + 1,
              orig_backup_path: origBackupPath || null,
              success_until_edit_path: successUntilPath,
              remaining_edits_path: remainingEditsPath,
              restore_to_original_failed: !!restoreError,
            },
          };
        }
      ...
      if (edit.oldText && typeof edit.newText === 'string') {
        for (let j = i + 1; j < cleanedArgs.edits.length; j += 1) {
          const next = cleanedArgs.edits[j];
          if (next && typeof next.oldText === 'string') {
            next.oldText = next.oldText
              .split(edit.oldText)
              .join(edit.newText);
          }
        }
    ...
    const successContent = [
      {
        type: 'text',
        text: `edit_file multi-edit pipeline applied ${cleanedArgs.edits.length} edits to ${basePath}.`,
      },
    ];
    ...
    try {
      ...
      console.log(
        '[MCP CONNECTOR] multi-edit SUCCESS diff: orig=%s current=%s',
        origForDiff || '(null)',
        currentForDiff || '(null)',
      );
      ...
      if (diffText && diffText.trim()) {
        ...
        console.log(
          '[MCP CONNECTOR] multi-edit SUCCESS diff: no diff text returned',
        );
      }
    } catch (e) {
      console.error('edit_file session diff (success path) error:', e);
    }
  ...
  // Generic git run_command sequencing: serialize ANY command that mentions
  // the substring "git". This may include false positives (e.g. words like
  // "legit"), which is acceptable—the primary goal is to prevent overlapping
  // git add/commit/push sequences.
  if (
    context.toolName === 'run_command' &&
    isGitRelatedOperation(context.toolName, context.toolArgs)
  ) {
    try {
      logRunCommandCleanse({
        stage: 'git-run-lock-enter',
        callId: context.callId,
        toolName,
        command:
          (context && context.toolArgs && context.toolArgs.command) ||
      ...
      console.error('git run-command lock enter log failure:', e);
    }
    return runGitRunCommandWithLock(clientEntry, context);
  }
