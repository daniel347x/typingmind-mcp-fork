...
const CONNECTOR_VERSION =
  '2025-12-08 - edit_file multi-edit always-apply + cascade-all-oldtexts + validator-summary + utf8-diff v11';
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (context.toolName === 'edit_file') {
    ...
    for (let i = 0; i < cleanedArgs.edits.length; i += 1) {
      ...
      // Success path: record and cascade oldText -> newText into all remaining edits.
    ...
    const succeededCount = successfulEdits.length;
