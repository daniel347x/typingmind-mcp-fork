...
const CONNECTOR_VERSION =
  '2026-09-23 - boot-time Workflowy nodes-cache refresh after cached auto-start (fire-and-forget) v20';
...
async function start(authToken) {
  ...
  try {
    ...
    if (cached && typeof cached === 'object') {
      ...
      if (wfBoot) {
        console.log('[MCP CONNECTOR] Boot-time Workflowy nodes-cache refresh (system cache.refresh) ...');
        wfBoot.client
          .callTool(
          ...
          .then((r) => {
            const text =
              r && Array.isArray(r.content) && r.content[0] && typeof r.content[0].text === 'string'
                ? r.content[0].text
                : '';
            const nodes = (text.match(/"node_count"\s*:\s*(\d+)/) || [])[1];
            if (r && r.isError) {
              console.warn(`⚠️ [MCP CONNECTOR] Boot-time cache refresh returned an error: ${text.slice(0, 300)}`);
            } else {
              console.log(`✅ [MCP CONNECTOR] Boot-time Workflowy cache refresh done${nodes ? ` (${nodes} nodes)` : ''}.`);
            }
          })
          .catch((e) => {
            console.warn('⚠️ [MCP CONNECTOR] Boot-time cache refresh failed (non-fatal):', e && e.message ? e.message : e);
          });
      }
    }
  } catch (e) {
