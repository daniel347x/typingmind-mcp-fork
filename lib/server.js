...
async function start(authToken) {
  ...
  app.post('/clients/:id/call_tools', auth, async (req, res) => {
    ...
    // 🚫 NUCLEAR OPTION: Block PowerShell for file safety
    if (name === 'run_command' && toolArgs && toolArgs.command) {
      ...
      if (cmd.includes('powershell') || cmd.includes('pwsh')) {
        console.log(`🚫 BLOCKED PowerShell command attempt: ${toolArgs.command}`);
        return res.status(403).json({
          content: [{
            type: 'text',
            text: '🚫 PowerShell blocked for file safety.\n\nPowerShell commands are disabled to prevent file corruption from backtick-n literal newlines.\n\nUse instead:\n- WSL: wsl bash -c "..."\n- Direct MCP tools: write_file, edit_file\n- CMD (if needed): cmd.exe /C "..."\n\nPowerShell\'s escape syntax (backtick-n) routinely corrupts files when agents use single-quoted strings.'
          }],
          isError: false
        });
      }
