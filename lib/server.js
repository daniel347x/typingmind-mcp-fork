...
const CONNECTOR_VERSION =
  '2026-01-26 - add sql_autopilot and multi-set exclusion locks v2';
...
async function runToolWithExclusionSetsLock(clientEntry, context, exclusionSets) {
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (Array.isArray(exclusionSets) && exclusionSets.length > 0) {
    ...
    return runToolWithExclusionSetsLock(clientEntry, context, exclusionSets);
  }
