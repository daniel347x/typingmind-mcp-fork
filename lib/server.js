...
// Window after the first /start during which multiple /start calls are
// treated as incremental and merged into the cache (superset of
// all observed servers). After this window, a /start is treated as
// a full replacement config for caching purposes.
const MCP_CACHE_MERGE_WINDOW_MS = 30_000;
let MCP_CACHE_FIRST_START_MS = null;
...
function saveCachedMcpServers(mcpServers) {
  try {
    ...
    let finalMap = mcpServers;
    const now = Date.now();

    // During the initial merge window after the FIRST /start seen by
    // this connector process, treat /start payloads as incremental and
    // merge them with any existing cached config. New definitions for a
    // given serverId win; servers not mentioned in the latest /start
    // remain.
    if (MCP_CACHE_FIRST_START_MS === null) {
      MCP_CACHE_FIRST_START_MS = now;
    }

    if (now - MCP_CACHE_FIRST_START_MS <= MCP_CACHE_MERGE_WINDOW_MS) {
      const existing = loadCachedMcpServers();
      if (existing && typeof existing === 'object') {
        finalMap = { ...existing, ...mcpServers };
      }
    }

    const payload = { mcpServers: finalMap };
    fs.writeFileSync(
