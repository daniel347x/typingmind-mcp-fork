...
async function runStructuredFileWithStage2Guard(clientEntry, context) {
  ...
  const lines = [
    ...
    '',
    'The file has been left in its post-edit state. Use the backup information above to inspect or restore.',
  ];
...
function isSafePowerShellCommand(rawCommand) {
  ...
  const hasAraxis =
    cmd.includes('start-process') && cmd.includes('araxis merge\\\\merge.exe');
