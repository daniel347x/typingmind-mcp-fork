...
const CONNECTOR_VERSION =
  '2026-01-26 - add sql_autopilot + multi-set exclusion locks, write_file/write_text_file gating, and connector-deploy git runner v5';
  ...
  },
  {
    ...
    tools: [
      // WSL cp from Obsidian MCP_Servers typingmind_mcp_connector into typingmind-mcp-fork repo
      {
        ...
        argKey: 'command',
        argPattern:
          /typingmind_mcp_connector\/lib\/server\.js.*typingmind-mcp-fork\/lib\/server\.js/i,
      },
      // Git commands in typingmind-mcp-fork repo (git add/commit/push)
      {
        toolName: 'run_command',
        argKey: 'command',
        argPattern: /typingmind-mcp-fork.*git\s+push/i,
      },
    ],
  },
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (
    ...
    const exclusionSetsForGit = findExclusionSetsForTool(
      context.toolName,
      context.toolArgs,
    );

    const runGit = async () => runGitRunCommandWithLock(clientEntry, context);

    if (
      Array.isArray(exclusionSetsForGit) &&
      exclusionSetsForGit.length > 0
    ) {
      const setIds = exclusionSetsForGit
        .map((s) => (s && typeof s.setId === 'string' ? s.setId : null))
        .filter((id) => !!id);
      try {
        logRunCommandCleanse({
        ...
        console.error(
          'exclusion-set detection log failure (git run_command):',
          e,
        );
      }
      return runToolWithExclusionSetsLock(
