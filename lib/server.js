...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  // Start-Process is non-blocking by default (unless -Wait is passed).
  // We use the explicit array syntax @(...) for ArgumentList to handle spaces safely.
  // Without @(...), PowerShell can sometimes split arguments on spaces despite quotes.
  // Final command shape:
  //   powershell -Command "Start-Process -FilePath 'C:\...\Merge.exe' -ArgumentList @('E:\...\Left.md', 'E:\...\Right.md')"
  return `powershell.exe -Command "Start-Process -FilePath '${araxisExe}' -ArgumentList @('${p1}', '${p2}')"`;
}
