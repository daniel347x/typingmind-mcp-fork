...
const CONNECTOR_VERSION =
  '2026-03-11 - polish edit_file diff headers for dry-run/current-file clarity v9';
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (context.toolName === 'edit_file') {
    ...
    const reportedPath = cleanedArgs.path || args.path;
    let effectivePath = reportedPath;
    let originalFileBuffer = null;
    let dryRunWorkingDir = null;
