  ...
  if (context.toolName === 'edit_file') {
        ...
        if (buf && buf.length > 0) {
          ...
          if (sessionHash && !sessionHashArg) {
            try {
              ...
              sessionEditsPath = path.join(sessionDir, 'edits_remaining.json');
            } catch (e) {
              console.error(
    ...
    for (let i = 0; i < cleanedArgs.edits.length; i += 1) {
      ...
      for (;;) {
        ...
        // If this edit is a no-op (oldText === newText), run at most one pass.
        if (edit.oldText === edit.newText) {
          break;
        }

        // Hard safety cap on passes per edit to prevent infinite loops.
        if (passes > EDIT_FILE_MAX_PASSES_PER_EDIT) {
          try {
            appendEditSessionLog(sessionDir, {
              ts: new Date().toISOString(),
              kind: 'max_passes_exceeded',
              session_hash: sessionHash || null,
              path: effectivePath || basePath,
              edit_index: i,
              pass_index: passes,
              max_passes: EDIT_FILE_MAX_PASSES_PER_EDIT,
            });
          } catch (e) {
            console.error('edit_file session log max_passes error:', e);
          }
          console.error(
            'edit_file multi-edit: max passes exceeded for edit',
            i,
            'session_hash',
            sessionHash,
          );
        ...
        try {
          fileText = await fs.promises.readFile(effectivePath, 'utf8');
        } catch (e) {
          console.error(
            'edit_file multi-edit: failed to read file for duplicate check:',
            effectivePath,
            e,
          );
          break;
        ...
        if (
          !edit.oldText ||
          typeof edit.oldText !== 'string' ||
          !fileText.includes(edit.oldText)
        ) {
          break;
        }
      }
