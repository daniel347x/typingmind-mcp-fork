...
const CONNECTOR_VERSION =
  '2026-07-20 - edit_file direct path honors replaceAll flag (replace-all + multi-match guard) v11';
...
async function runStructuredFileWithStage2Guard(clientEntry, context) {
  ...
  // occurrence of oldText; when false/absent, single-match semantics apply and
  // more than one match is treated as an error (never silent first-match-only).
    ...
    if (useDirectReplaceAll) {
      ...
      if (
        ...
        if (occurrences === 0) {
              ...
              if (
                ...
                if (occurrencesFlex > 0) {
                  if (!replaceAllRequested && occurrencesFlex > 1) {
                    // Multiple matches but replaceAll not set: error, do not modify.
                    toolResult = {
                      isError: true,
                      content: [
                        {
                          type: 'text',
                          text:
                            `edit_file (python indent-flex): ${occurrencesFlex} occurrences of oldText found in ${displayPath}, ` +
                            `but replaceAll was not set. Pass replaceAll:true to replace all occurrences, ` +
                            `or make oldText unique. The file was NOT modified.`,
                        },
                      ],
                      directReplaceAll: {
                        occurrences: occurrencesFlex,
                        pythonIndentFlex: true,
                        multiMatchNoReplaceAll: true,
                      },
                    };
                    autoFixed = true;
                  } else {
          ...
          // Multiple matches but replaceAll not set: per the edit_file schema this
          // is an error. Do NOT modify the file (previously this silently did
          // first-match-only, which the schema now forbids).
          toolResult = {
            isError: true,
            content: [
              {
                type: 'text',
                text:
                  `edit_file: ${occurrences} occurrences of oldText found in ${displayPath}, ` +
                  `but replaceAll was not set. Pass replaceAll:true to replace all occurrences, ` +
                  `or make oldText unique. The file was NOT modified.`,
              },
            ],
