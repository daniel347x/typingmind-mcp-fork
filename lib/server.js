...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  const escapeDoubleQuotes = (p) => String(p).replace(/"/g, '""');

  const p1 = escapeDoubleQuotes(leftPath);
  const p2 = escapeDoubleQuotes(rightPath);

  // Use CMD quoting (same strategy as the WindSurf cleanser) to avoid
  // PowerShell parsing issues with spaces and escaping.
  return `cmd /c ""${araxisExe}" "${p1}" "${p2}""`;
}
...
function transformAraxisCommand(rawCommand) {
  ...
  // Fallback: if we didn't find two file paths via the conservative scanner,
  ...
  const leftPath = filePaths[filePaths.length - 2];
  const rightPath = filePaths[filePaths.length - 1];
