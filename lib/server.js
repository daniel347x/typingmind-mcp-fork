...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  // Escape single quotes inside paths for PowerShell single-quoted strings.
  const escapeSingleQuotes = (p) => String(p).replace(/'/g, "''");

  const p1 = escapeSingleQuotes(leftPath);
  const p2 = escapeSingleQuotes(rightPath);

  const inner =
    `Start-Process -FilePath '${araxisExe}' -ArgumentList @('${p1}','${p2}')`;

  // We deliberately avoid double quotes inside `inner` so the outer
  // -Command "..." wrapper is stable.
  return `powershell.exe -Command "${inner}"`;
}
...
function transformAraxisCommand(rawCommand) {
  ...
  const looksLikeAraxis =
    lower.includes('start-process') &&
    lower.includes('araxis merge\\merge.exe') &&
  ...
  // Filter for Markdown files only (.md or .original.md) so we skip the .exe path.
  let mdPaths = quotedPaths
    .map((q) => q.path)
    .filter((p) => /\.md$/i.test(p) || /\.original\.md$/i.test(p));

  // Fallback: if we didn't find two .md paths via the conservative scanner,
  // look for double-quoted Windows paths inside the command, even if they
  // are nested inside single-quoted segments.
  if (mdPaths.length < 2) {
    const looseMatches = [];
    const re = /"([A-Za-z]:\\\\[^\"]+?\.(?:original\.md|md))"/gi;
    let m;
    while ((m = re.exec(rawCommand)) !== null) {
      looseMatches.push(m[1]);
    }
    if (looseMatches.length >= 2) {
      mdPaths = looseMatches;
    }
  }
  ...
  if (mdPaths.length < 2) {
    logRunCommandCleanse({
      ...
      reason: 'could-not-find-two-md-paths',
      discoveredPaths: quotedPaths,
    });
    return { changed: false, command: rawCommand };
  }

  const leftPath = mdPaths[mdPaths.length - 2];
  const rightPath = mdPaths[mdPaths.length - 1];
...
/**
 * Build a canonical WindSurf PowerShell command that is robust
 * against spaces in file paths.
 *
 * Uses -ArgumentList @('-r','path') form for safe quoting.
...
function buildWindSurfCommand(filePath) {
  ...
  // Escape single quotes inside path for PowerShell single-quoted strings.
  const escapeSingleQuotes = (p) => String(p).replace(/'/g, "''");
  const safePath = escapeSingleQuotes(filePath);

  const inner = `Start-Process -FilePath '${windsurfExe}' -ArgumentList @('-r','${safePath}')`;

  // Avoid double quotes inside `inner` so outer -Command "..." wrapper is stable.
  return `powershell.exe -Command "${inner}"`;
}
