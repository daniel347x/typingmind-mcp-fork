...
// Window after process start during which multiple /start calls are
...
const MCP_CONNECTOR_BOOT_TIME_MS = Date.now();
...
function saveCachedMcpServers(mcpServers) {
  try {
    ...
    // During the initial merge window after connector boot, treat
    // /start payloads as incremental and merge them with any
    // existing cached config. New definitions for a given serverId
    // win; servers not mentioned in the latest /start remain.
    if (now - MCP_CONNECTOR_BOOT_TIME_MS <= MCP_CACHE_MERGE_WINDOW_MS) {
