...
const CONNECTOR_VERSION =
  '2025-12-12 - edit_file no-match as failure + existence-check warnings + indent-flex v16';
...
function findUniquePythonIndentFlexMatch(fileText, oldText) {
  ...
  for (let i = 0; i + L <= fileLines.length; i += 1) {
    ...
    if (dedentedBlock === dedentedOldBlock) {
      const startLine = i;
      const endLine = i + L - 1;
      const startChar = lineStarts[startLine] + blockBaseIndent;
            ...
            try {
              ...
              if (
                ...
                if (occurrencesFlex > 0) {
                  const updatedTextFlex = partsFlex.join(edit.newText);
                    ...
                    content: [
                      {
                        type: 'text',
                        text:
                          `edit_file direct replace-all (python indent-flex) applied ${occurrencesFlex} occurrence` +
          ...
          if (!autoFixed) {
            toolResult = {
              ...
              content: [
                {
                  type: 'text',
                  text: `edit_file direct replace-all: NO MATCH – 0 occurrences of oldText found in ${filePath}. The file was NOT modified.`,
                },
              ],
              directReplaceAll: {
                occurrences,
...
//     * toolName: Exact tool name to match (required for now)
//     * argKey: Optional - which argument to check (future)
//     * argPattern: Optional - pattern to match in argument (future)
//     * disallowedPaths: Optional - specific file paths to block (future)
//
...
const TOOL_EXCLUSION_SETS = [
  {
    ...
    description: 'NEXUS JEWELSTORM anchor + weave operations (same tag conflict)',
    tools: [
      { toolName: 'nexus_anchor_jewels' },
      { toolName: 'nexus_weave_enchanted_async' },
    ],
  },
 ...
 *
 * For now: only checks toolName (exact match).
 * Future: can check argKey, argPattern, disallowedPaths.
 */
function toolMatchesExclusionMatcher(toolName, toolArgs, matcher) {
  // Currently only tool name matching is implemented
  if (matcher.toolName && matcher.toolName !== toolName) {
    return false;
  }

  // Future: check argKey/argPattern/disallowedPaths here
  // if (matcher.argKey && toolArgs) {
  //   const argValue = toolArgs[matcher.argKey];
  //   if (matcher.argPattern && !argPattern.test(argValue)) {
  //     return false;
  //   }
  // }
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  if (context.toolName === 'edit_file') {
    ...
    for (let i = 0; i < cleanedArgs.edits.length; i += 1) {
      ...
      if (edit.oldText && typeof edit.newText === 'string') {
        for (let j = 0; j < cleanedArgs.edits.length; j += 1) {
    ...
    if (hasRealErrors) {
      ...
      summaryLines.push(`ERRORS: 0 out of ${totalEdits}`);
    }
  ...
  const exclusionSet = findExclusionSetForTool(context.toolName, context.toolArgs);
  if (exclusionSet) {
    try {
      logRunCommandCleanse({
        ...
        callId: context.callId,
        toolName: context.toolName,
        exclusionSetId: exclusionSet.setId,
        ts: new Date().toISOString(),
      });
    } catch (e) {
      console.error('exclusion-set detection log failure:', e);
    }
    return runToolWithExclusionSetLock(clientEntry, context, exclusionSet);
  }
