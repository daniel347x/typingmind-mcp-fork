...
const CONNECTOR_VERSION =
  '2026-09-23 - boot-time cache refresh: timestamps, elapsed, epoch + snapshot, in-band failure detection, 15-min timeout v21';
...
function rewriteUnifiedDiffHeaders(diffText, origLabel, currentLabel) {
  if (typeof diffText !== 'string' || !diffText) return diffText;

  const hasCrLf = diffText.includes('\r\n');
  const lines = diffText.split(/\r?\n/);
  if (!lines.length) return diffText;

  if (typeof origLabel === 'string' && origLabel.length > 0 && lines[0].startsWith('--- ')) {
    lines[0] = `--- ${origLabel}`;
  }
  if (
...
async function handleFileIoPreflight(context) {
  ...
  if (context && context.toolName === 'edit_file') {
    ...
    const isDryRunRequested =
      args.dryRun === true || args.dry_run === true;

    if (
      isDryRunRequested &&
      requestedPath &&
      !context.dryRunWorkingPath
    ) {
      try {
        const dryRunClone = await createDryRunWorkingCopy(requestedPath);
        context.dryRunOriginalPath = requestedPath;
        context.dryRunWorkingPath = dryRunClone.workingPath;
        context.dryRunWorkingDir = dryRunClone.tempDir;
        context.toolArgs = {
          ...args,
          path: dryRunClone.workingPath,
        };
      } catch (e) {
        console.error(
          'edit_file dryRun preflight: failed to prepare temporary working copy:',
          requestedPath,
          e,
        );
        throw e;
...
async function runStructuredFileWithStage2Guard(clientEntry, context) {
  const args = context.toolArgs || {};
  const filePath = args.path;
  const displayPath = context.displayPath || filePath;
  ...
  // Deferred-validation mode (edit_file multi-edit shadow passes): callers
  // that validate ONCE at the end of a batch skip per-edit backups entirely.
  const skipStage2Validation =
    context && context.skipStage2Validation === true;
  // When this edit targets a temporary shadow working copy, corrupted-file
  // artifacts are redirected to the REAL file's directory via this base path.
  const artifactBasePath =
    (context &&
      typeof context.stage2ArtifactBasePath === 'string' &&
      context.stage2ArtifactBasePath) ||
    filePath;

  let backupInfo = null;
  let backupPath = null;
  if (!skipStage2Validation) {
    backupInfo = await createStage2Backup(filePath, kind);
    backupPath = backupInfo && backupInfo.backupPath;
  }
  ...
  // replaceAll is a TOP-LEVEL edit_file arg (per the tool schema), preserved on
  // toolArgs through cleanedArgs -> singleContext. When true, replace EVERY
  // occurrence of oldText. When false/absent, oldText must match EXACTLY ONE
  // occurrence: zero matches keeps the existing no-match behavior, and MORE
  // than one match is a visible, explanatory error with the file left
  // unchanged (matches the advertised edit_file tool contract; restored v13).
  const replaceAllRequested = args && args.replaceAll === true;
  ...
  try {
    if (useDirectReplaceAll) {
      ...
      if (
        ...
        if (occurrences === 0) {
            ...
            try {
              ...
              if (
                ...
                if (occurrencesFlex > 0) {
                  if (!replaceAllRequested && occurrencesFlex > 1) {
                    // Multi-match guard (v13), python indent-flex branch:
                    // replaceAll false/absent with >1 match => visible,
                    // explanatory error; the file is NOT modified.
                    toolResult = {
                      isError: true,
                      content: [
                        {
                          type: 'text',
                          text:
                            `edit_file direct first-match (python indent-flex): MULTIPLE MATCHES – ${occurrencesFlex} occurrences of oldText found in ${displayPath} (after python indent-flex auto-fix), but replaceAll is false/absent.\n\n` +
                            'When replaceAll is not set to true, oldText must match EXACTLY ONE location; with more than one match the edit is rejected and the file is left UNCHANGED (no occurrences were replaced).\n\n' +
                            'To proceed, either:\n' +
                            `  1. Re-run this edit with replaceAll: true to replace ALL ${occurrencesFlex} occurrences in one pass; or\n` +
                            '  2. Make oldText more specific (include more surrounding context) so it matches exactly one location, then re-run.\n\n' +
                            'Note: the legacy "call edit_file repeatedly to replace one occurrence at a time" pattern is not supported here – use replaceAll: true instead.',
                        },
                      ],
                      directReplaceAll: {
                        occurrences: occurrencesFlex,
                        replaced: 0,
                        replaceAll: replaceAllRequested,
                        pythonIndentFlex: true,
                        multiMatchError: true,
                      },
                    };
                  } else {
                    const updatedTextFlex = replaceAllRequested
                      ? partsFlex.join(edit.newText)
                      : partsFlex[0] + edit.newText + partsFlex.slice(1).join(edit.oldText);
                    await fs.promises.writeFile(filePath, updatedTextFlex, 'utf8');

                    const replacedCountFlex = replaceAllRequested ? occurrencesFlex : 1;
                    toolResult = {
                      isError: false,
                      content: [
                        {
                          type: 'text',
                          text:
                            `edit_file direct ${replaceAllRequested ? 'replace-all' : 'first-match'} (python indent-flex) replaced ${replacedCountFlex} of ${occurrencesFlex} occurrence` +
                            `${occurrencesFlex === 1 ? '' : 's'} of oldText in ${displayPath}.`,
                        },
                      ],
                      directReplaceAll: {
                        occurrences: occurrencesFlex,
                        replaced: replacedCountFlex,
                        replaceAll: replaceAllRequested,
                        pythonIndentFlex: true,
                      },
                    };
          ...
          if (!autoFixed) {
            toolResult = {
              ...
              content: [
                {
                  type: 'text',
                  text: `edit_file direct first-match: NO MATCH – 0 occurrences of oldText found in ${displayPath}. The file was NOT modified.`,
                },
              ],
              directReplaceAll: {
                occurrences,
        ...
        } else if (!replaceAllRequested && occurrences > 1) {
          // Multi-match guard (v13): replaceAll false/absent with >1 match =>
          // visible, explanatory error; the file is NOT modified. This matches
          // the advertised edit_file tool contract ("false or null -> if
          // multiple matches are found, an error is returned").
          toolResult = {
            isError: true,
            content: [
              {
                type: 'text',
                text:
                  `edit_file direct first-match: MULTIPLE MATCHES – ${occurrences} occurrences of oldText found in ${displayPath}, but replaceAll is false/absent.\n\n` +
                  'When replaceAll is not set to true, oldText must match EXACTLY ONE location; with more than one match the edit is rejected and the file is left UNCHANGED (no occurrences were replaced).\n\n' +
                  'To proceed, either:\n' +
                  `  1. Re-run this edit with replaceAll: true to replace ALL ${occurrences} occurrences in one pass; or\n` +
                  '  2. Make oldText more specific (include more surrounding context) so it matches exactly one location, then re-run.\n\n' +
                  'Note: the legacy "call edit_file repeatedly to replace one occurrence at a time" pattern is not supported here – use replaceAll: true instead.',
              },
            ],
          ...
          const updatedText = replaceAllRequested
            ? parts.join(edit.newText)
            : parts[0] + edit.newText + parts.slice(1).join(edit.oldText);
          await fs.promises.writeFile(filePath, updatedText, 'utf8');

          const replacedCount = replaceAllRequested ? occurrences : 1;
          toolResult = {
            isError: false,
            content: [
              {
                type: 'text',
                text: `edit_file direct ${
                  replaceAllRequested ? 'replace-all' : 'first-match'
                } replaced ${replacedCount} of ${occurrences} occurrence${
                  occurrences === 1 ? '' : 's'
                } of oldText in ${displayPath}.`,
              },
            ],
            directReplaceAll: {
              occurrences,
  ...
  const validation = skipStage2Validation
    ? { ok: true, skipped: true }
    : await validateStructuredFile(filePath, kind);
  ...
  if (backupPath) {
    try {
      const dir = path.dirname(artifactBasePath);
      const base = path.basename(artifactBasePath);
      const stamp = new Date().toISOString().replace(/[:.]/g, '-');
      const rand = Math.random().toString(16).slice(2, 8);
  ...
  // Put the absolute corrupt-snapshot path up front and on its own lines so it
  // survives multi-edit error_preview truncation AND is scannable in the full
  // validator report (agents were missing it buried mid-prose).
  const corruptSnapshotLines = corruptPath
    ? [
        'INVALID POST-EDIT SNAPSHOT (absolute path — open this file to see the rejected state):',
        corruptPath,
        '',
      ]
    : [
        'INVALID POST-EDIT SNAPSHOT: (could not save a corrupted post-edit copy)',
        '',
      ];
  ...
  const lines = [
    ...
    '',
    ...corruptSnapshotLines,
    `Path (target file, now restored): ${displayPath}`,
    backupLine,
    restoreLine,
    '',
    `Validator error: ${errorMessage}`,
    '',
    'The invalid edit has been rolled back to the last known-good version. Inspect the INVALID POST-EDIT SNAPSHOT path above if you want to see exactly what the rejected edit produced.',
  ];
...
//     * callPattern: Optional - RegExp tested against full call text (toolName + JSON.stringify(toolArgs))
...
const TOOL_EXCLUSION_SETS = [
  ...
  },
  {
    ...
    tools: [
      ...
      // db-autopilot execution step on A100. Preserve the historical
      // ssh-mcp `exec` name and support its current `run-command` name.
      {
        toolName: 'exec',
        argKey: 'command',
  ...
  {
    setId: 'obsidian-bases',
    description:
      'Obsidian Semantic Bases workflows: serialize base-file write + bases tool calls to prevent file-not-found races when agents call write_file + bases in parallel.',
    tools: [
      // Match any tool call whose tool name or serialized arguments contain the word "bases".
      { callPattern: /\bbases\b/i },
    ],
  },
  {
 ...
 *  - toolNameContains: case-insensitive substring match on tool name (optional)
 *  - callPattern: regex tested against full call text (toolName + JSON.stringify(toolArgs)) (optional)
...
function toolMatchesExclusionMatcher(toolName, toolArgs, matcher) {
  ...
  // Full-call regex match (toolName + JSON.stringify(toolArgs))
  if (matcher.callPattern) {
    let callText = '';
    try {
      const argsText = JSON.stringify(toolArgs === undefined ? null : toolArgs);
      callText = `${toolName || ''} ${argsText || ''}`;
    } catch (e) {
      // If args cannot be stringified, fail this matcher.
      return false;
    }
    try {
      // Protect against /g regex lastIndex surprises.
      matcher.callPattern.lastIndex = 0;
      if (!matcher.callPattern.test(callText)) {
        return false;
      }
    } catch (e) {
      console.error('exclusion-set callPattern test failure:', e);
      return false;
    }
  }
...
async function createDryRunWorkingCopy(targetPath, seedBuffer) {
  const parsed = path.parse(targetPath || 'edit-file-dryrun.txt');
  const tempDir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), 'mcp-edit-file-dryrun-'),
  );
  const baseName = parsed.base && parsed.base.length > 0
    ? parsed.base
    : `working${parsed.ext || '.tmp'}`;
  const workingPath = path.join(tempDir, baseName);

  if (Buffer.isBuffer(seedBuffer)) {
    await fs.promises.writeFile(workingPath, seedBuffer);
  } else {
    await fs.promises.copyFile(targetPath, workingPath);
  }

  return { tempDir, workingPath };
}

async function cleanupDryRunWorkingCopy(tempDir) {
  if (!tempDir) return;
  try {
    await fs.promises.rm(tempDir, { recursive: true, force: true });
  } catch (e) {
    console.error(
      'edit_file dryRun: failed to clean up temporary working copy:',
      tempDir,
      e,
    );
  }
...
// ============================================================================
// edit_file multi-edit: deferred-validation dual path (v14, 2026-07-29)
// ----------------------------------------------------------------------------
// Problem: agents frequently compose a BATCH of edits that are individually
// syntax-breaking but collectively valid (e.g. one edit rewrites the top of a
// block, a later edit rewrites the bottom). Legacy per-edit Stage 2 validation
// rejected those batches at the first intermediate (invalid) state.
//
// Design (default-on for multi-edit batches; strict_per_edit: true opts out):
//   PASS 1 (lenient): run every edit against a PRIVATE shadow working copy
//     with per-edit validation skipped. If every edit matched/applied cleanly
//     AND the shadow validates ONCE at the end, commit the shadow over the
//     real file and report full success (the strict path is never run).
//   PASS 2 (strict fallback): otherwise re-run the batch on a FRESH shadow
//     with legacy per-edit validation (backup + rollback per edit, now on the
//     shadow), commit the shadow's final state, and report exactly the
//     legacy per-edit outcomes.
// The real file is therefore written AT MOST ONCE per batch (single commit),
// and the whole batch runs under a batch-level hold of the per-path lock, so
// concurrent batches on the same file serialize at batch granularity instead
// of interleaving per-edit writes.
//
// NOTE (legacy duplication): the inline loop + reporting in dispatchToolCall
// is the FROZEN legacy path, still used for dryRun, single-edit batches, and
// strict_per_edit: true. runMultiEditLoop / buildMultiEditReport are the
// shared helpers used by the dual path. A future cleanup can route the legacy
// path through the same helpers and delete the inline copy.
// ============================================================================

async function commitShadowToReal(shadowPath, realPath, seedBuffer, force) {
  // Byte-compare the live file against the seed the shadow was built from:
  // if it changed under us (external writer), report raced=true. Unless
  // force is set, a raced commit is refused so the caller can re-seed/retry.
  let raced = false;
  if (Buffer.isBuffer(seedBuffer)) {
    let live = null;
    try {
      live = await fs.promises.readFile(realPath);
    } catch (e) {
      live = null;
    }
    if (live && !live.equals(seedBuffer)) {
      raced = true;
    }
  }
  if (raced && !force) {
    return { committed: false, raced };
  }
  try {
    await fs.promises.copyFile(shadowPath, realPath);
  } catch (e) {
    console.error('edit_file shadow commit failed:', shadowPath, realPath, e);
    return { committed: false, raced, error: String(e) };
  }
  return { committed: true, raced };
}

async function runEditFileBatchLocked(clientEntry, context, batchFn) {
  // Same promise-chain discipline as runStructuredFileWithStage2GuardLocked,
  // but the locked unit is the WHOLE multi-edit batch (shadow passes + the
  // single commit), so connector-mediated edits on the same path serialize
  // at batch granularity. Callers inside batchFn must NOT re-acquire the
  // per-path lock (per-edit shadow work needs no lock) - that would deadlock.
  const args = (context && context.toolArgs) || {};
  const filePath = args.path;
  const key = getEditFileLockKey(filePath);
  if (!key) {
    return batchFn();
  ...
  const previous = EDIT_FILE_LOCKS.get(key) || Promise.resolve();
  const callId = (context && context.callId) || 'unknown';
  const chained = previous
    .catch(() => {
      // Swallow prior errors so the chain never breaks.
    })
    .then(async () => {
      try {
        logRunCommandCleanse({
          stage: 'edit-file-batch-lock-run',
          callId,
          path: filePath,
          lockKey: key,
          ts: new Date().toISOString(),
        ...
        console.error('edit-file batch lock run log failure:', e);
      }
      // Same spacing discipline as the per-edit lock (Windows EPERM avoidance).
      await new Promise((resolve) => setTimeout(resolve, 500));
      return batchFn();
    })
    .finally(() => {
...
async function runMultiEditLoop(clientEntry, context, S) {
  // Executes one pass of the multi-edit loop against S.effectivePath (which
  // may be the real file or a private shadow working copy).
  // S: { cleanedArgs, reportedPath, basePath, effectivePath, sessionDir,
  //      sessionHash, sessionSuccessPath, sessionEditsPath,
  //      skipPerEditValidation, lockPerEdit, passLabel }
  const successfulEdits = [];
  const failedEdits = [];
  const noOpEdits = [];
  const existenceChecks = [];
  let remainingEditsPath = S.sessionEditsPath || null;

  for (let i = 0; i < S.cleanedArgs.edits.length; i += 1) {
    const edit = S.cleanedArgs.edits[i];
    ...
    if (isExistenceCheck) {
      ...
      try {
        appendEditSessionLog(S.sessionDir, {
          ts: new Date().toISOString(),
          kind: 'existence_check',
          session_hash: S.sessionHash || null,
          path: S.effectivePath || S.basePath,
          edit_index: i,
          found,
          pass: S.passLabel || null,
        });
      } catch (e) {
    ...
    const singleContext = {
      ...
      displayPath: S.reportedPath || S.basePath,
      skipStage2Validation: S.skipPerEditValidation === true,
      stage2ArtifactBasePath: S.reportedPath || S.basePath,
      toolArgs: {
        ...S.cleanedArgs,
    ...
    const result = S.lockPerEdit
      ? await runStructuredFileWithStage2GuardLocked(clientEntry, singleContext)
      : await runStructuredFileWithStage2Guard(clientEntry, singleContext);
    passes += 1;
    ...
    if (result && result.isError) {
      ...
      failedEdits.push({
        ...
        stage2_validation_failed: !!(result && result.stage2Validation),
        stage2_corrupt_path:
          result &&
          result.stage2Validation &&
          typeof result.stage2Validation.corruptPath === 'string'
            ? result.stage2Validation.corruptPath
            : null,
      });

      if (S.effectivePath && S.sessionSuccessPath) {
        try {
          await fs.promises.copyFile(S.effectivePath, S.sessionSuccessPath);
        } catch (e) {
          console.error(
            'edit_file multi-edit: failed to write success-until checkpoint:',
            S.sessionSuccessPath,
            e,
          );
        }
      }
    ...
    // Success path: distinguish no-op vs change.
    const occurrences =
      result &&
    ...
    if (occurrences === 0) {
      ...
      // No match: file did not change for this edit.
      continue;
    }
  ...
  if (failedEdits.length > 0) {
    try {
      const failedSpec = {
        path: S.reportedPath || S.basePath,
        failed_indices: failedEdits.map((e) => e.index),
        edits: failedEdits.map((e) => {
          const originalEdit = S.cleanedArgs.edits[e.index];
          return originalEdit && typeof originalEdit === 'object'
            ? { ...originalEdit }
            : originalEdit;
        }),
      };
...
async function buildMultiEditReport(S) {
  // S: { cleanedArgs, reportedPath, basePath, isDryRunRequested, sessionHash,
  //      sessionDir, sessionOrigPath, origBackupPath, currentForDiffPath,
  //      successfulEdits, failedEdits, noOpEdits, existenceChecks,
  //      remainingEditsPath, provenanceLines, dryRunDeferredHint,
  //      racedWarning, pathTaken }
  const successfulEdits = S.successfulEdits || [];
  const failedEdits = S.failedEdits || [];
  const noOpEdits = S.noOpEdits || [];
  const existenceChecks = S.existenceChecks || [];
  const remainingEditsPath = S.remainingEditsPath || null;
  const reportedPath = S.reportedPath;
  const basePath = S.basePath;
  const isDryRunRequested = S.isDryRunRequested === true;
  ...
  const totalEdits = S.cleanedArgs.edits.length;
  ...
  summaryLines.push(
    `edit_file multi-edit pipeline completed ${totalEdits} edits on ${reportedPath || basePath}.`,
  );
  if (isDryRunRequested) {
    summaryLines.push(
      `🧪 DRY RUN: enhanced edit_file executed against a temporary working clone; the original file at ${reportedPath || basePath} was not modified.`,
    );
  }
  if (Array.isArray(S.provenanceLines)) {
    for (const line of S.provenanceLines) {
  ...
  if (hasRealErrors) {
    ...
    if (hasExistenceWarnings) {
      ...
      // Successes without a green checkmark when existence check failed
      summaryLines.push(
  ...
  if (failedCount) {
    ...
    for (const e of failedEdits) {
      ...
      if (
        e.stage2_corrupt_path &&
        typeof e.stage2_corrupt_path === 'string'
      ) {
        lines.push(
          `    invalid_post_edit_snapshot: ${e.stage2_corrupt_path}`,
        );
      }
      if (e.error_preview) {
  ...
  try {
    const origForDiff = S.sessionOrigPath || S.origBackupPath;
    const currentForDiff = S.currentForDiffPath;
    console.log(
      '[MCP CONNECTOR] multi-edit FINAL diff: orig=%s current=%s',
      origForDiff || '(null)',
      currentForDiff || '(null)',
    );
    let diffText = await computeSessionDiffText(origForDiff, currentForDiff);
    if (diffText && diffText.trim()) {
      const userFacingOrigLabel = reportedPath
        ? `${reportedPath} (original snapshot)`
        : origForDiff;
      const userFacingCurrentLabel = reportedPath
        ? `${reportedPath}${isDryRunRequested ? ' (dry-run preview)' : ' (current file)'}`
        : currentForDiff;
      diffText = rewriteUnifiedDiffHeaders(
        diffText,
        userFacingOrigLabel,
        userFacingCurrentLabel,
      );
      successContent.push({
        type: 'text',
        text:
  ...
  if (failedCount) {
    ...
    for (const e of failedEdits) {
      ...
      const corruptHeader =
        e.stage2_corrupt_path &&
        typeof e.stage2_corrupt_path === 'string'
          ? `INVALID POST-EDIT SNAPSHOT (absolute path):\n${e.stage2_corrupt_path}\n\n`
          : '';
      validatorLines.push(
  ...
  if (S.dryRunDeferredHint === true) {
    successContent.push({
      type: 'text',
      text:
        '\n⚠️ DRY RUN LIMITATION: dryRun validates after EVERY individual edit, so it CANNOT preview batches whose INTERMEDIATE states are invalid but whose FINAL state is valid (edits intentionally composed so they only parse once combined). At least one edit in this dry run failed Stage 2 validation. If that composition was intentional, re-run this same edit_file call WITHOUT dryRun: the live multi-edit pipeline now defers validation to the END of the batch by default and will accept the batch if the final file validates. (Pass strict_per_edit: true on the live call to force legacy per-edit validation.)',
    });
  }

  if (S.racedWarning === true) {
    successContent.push({
      type: 'text',
      text:
        '\n⚠️ CONCURRENT-MODIFICATION WARNING: the target file changed externally while this batch was running (not via this connector). The batch result was committed over those external changes after one automatic re-seed/retry. Review the diff above carefully.',
    });
  }
  ...
  const finalResult = {
    ...
    multiEdit: {
      ...
      tracking_dir: S.sessionDir,
      tracking_orig_path: S.sessionOrigPath,
      success_count: succeededCount,
      failed_count: failedCount,
      remaining_edits_path: remainingEditsPath || null,
      dry_run: isDryRunRequested,
      path_taken: S.pathTaken || null,
    },
  };
...
async function runMultiEditOrchestrator(clientEntry, context, S) {
  // Dual-path multi-edit driver. S carries the session state prepared by the
  // edit_file branch (see the call site for the field list).
  const kind = getStructuredFileKind(S.reportedPath);

  return runEditFileBatchLocked(clientEntry, context, async () => {
    let seedBuffer = S.originalFileBuffer;

    for (let attempt = 0; attempt < 2; attempt += 1) {
      const finalAttempt = attempt === 1;

      // ---- PASS 1 (lenient): every edit on a private shadow, no per-edit validation.
      const shadow1 = await createDryRunWorkingCopy(
        S.reportedPath,
        seedBuffer,
      );
      let r1;
      ...
      let raced1 = false;
      try {
        r1 = await runMultiEditLoop(clientEntry, context, {
          ...S,
          effectivePath: shadow1.workingPath,
          skipPerEditValidation: true,
          lockPerEdit: false,
          passLabel: 'deferred-validation',
        });
        const existenceOk = r1.existenceChecks.every(
          (e) => e && e.found !== false,
        );
        const clean =
          r1.failedEdits.length === 0 &&
          r1.noOpEdits.length === 0 &&
          existenceOk;
        if (clean) {
          const validation = await validateStructuredFile(
            shadow1.workingPath,
            kind,
          );
          if (validation && validation.ok) {
            const commit = await commitShadowToReal(
              shadow1.workingPath,
              S.reportedPath,
              seedBuffer,
              finalAttempt,
            );
            accept = commit.committed === true;
            raced1 = commit.raced === true;
          }
        }
      } finally {
        await cleanupDryRunWorkingCopy(shadow1.tempDir);
      }
      ...
      if (accept) {
        try {
          appendEditSessionLog(S.sessionDir, {
            ts: new Date().toISOString(),
            kind: 'path_taken',
            path_taken: 'deferred_validation_accept',
            attempt,
            raced: raced1,
          });
        } catch (e) {
          console.error('edit_file session log path_taken error:', e);
        }
        return buildMultiEditReport({
          ...S,
          ...r1,
          currentForDiffPath: S.reportedPath,
          pathTaken: 'deferred_validation_accept',
          provenanceLines: [
            '✅ DEFERRED-VALIDATION ACCEPT: per-edit validation was deferred for this batch; all edits were applied to a shadow copy and the final file was validated ONCE after the last edit. It is structurally clean. (Session log records path_taken=deferred_validation_accept.)',
          ],
          racedWarning: raced1 && finalAttempt,
        });
      }

      // If the file changed under us during pass 1 and we can still retry,
      // re-seed and start over; otherwise fall through to the strict pass.
      if (raced1 && !finalAttempt) {
        try {
          seedBuffer = await fs.promises.readFile(S.reportedPath);
        } catch (e) {
          console.error('edit_file dual-path re-seed failed:', e);
        }
        continue;
      }

      // ---- PASS 2 (strict fallback): legacy per-edit validation, still on a
      // private shadow; the real file is written exactly once, at commit.
      const shadow2 = await createDryRunWorkingCopy(
        S.reportedPath,
        seedBuffer,
      );
      let r2;
      let commit2 = { committed: false, raced: false };
      try {
        r2 = await runMultiEditLoop(clientEntry, context, {
          ...S,
          effectivePath: shadow2.workingPath,
          skipPerEditValidation: false,
          lockPerEdit: false,
          passLabel: 'strict-shadow',
        });
        commit2 = await commitShadowToReal(
          shadow2.workingPath,
          S.reportedPath,
          seedBuffer,
          finalAttempt,
        ...
        await cleanupDryRunWorkingCopy(shadow2.tempDir);
      }

      if (!commit2.committed && !finalAttempt) {
        try {
          seedBuffer = await fs.promises.readFile(S.reportedPath);
        } catch (e) {
          console.error('edit_file dual-path re-seed failed:', e);
          ...
          content: [
            {
              type: 'text',
              text:
                `🚫 edit_file dual-path: failed to commit the shadow working copy over ${S.reportedPath}: ${commit2.error || '(raced / unknown)'}\n\n` +
                'The real file was NOT modified by this batch. Re-run the edit_file call to retry.',
            },
          ],
        };
      }
      ...
      try {
        appendEditSessionLog(S.sessionDir, {
          ts: new Date().toISOString(),
          kind: 'path_taken',
          path_taken: 'strict_shadow_fallback',
          attempt,
          raced: commit2.raced === true,
        });
      } catch (e) {
        console.error('edit_file session log path_taken error:', e);
      }
      ...
      return buildMultiEditReport({
        ...
        ...r2,
        currentForDiffPath: S.reportedPath,
        pathTaken: 'strict_shadow_fallback',
        provenanceLines: [
          'ℹ️ DEFERRED-VALIDATION FALLBACK: the batch did not validate cleanly as a whole (or had match/existence-check failures), so it was executed with legacy per-edit validation on a shadow copy and committed once. The per-edit outcomes below are authoritative (unchanged from legacy behavior).',
        ],
        racedWarning: commit2.raced === true && finalAttempt,
      });
    }

    // Unreachable in practice (the loop always returns); keep a loud guard.
    return {
      isError: true,
      content: [
        {
          type: 'text',
          text:
            'edit_file dual-path orchestrator exhausted its retry loop without producing a result. This is a connector bug; please report it.',
        },
      ],
    };
  });
...
// ═══════════════════════════════════════════════════════════════════════════
// SCHEMA-DRIVEN WIRE-FORMAT ARG REPAIR (v18, 2026-09-10)
// ═══════════════════════════════════════════════════════════════════════════
// Some model routes (observed: Qwen via TypingMind) serialize typed JSON
// scalars as STRINGS on the wire: boolean `true` arrives as "True", integer
// `90` arrives as "90". The values are intact but mistyped, so downstream
// strict checks (`=== true`, Rust u64 deserialization) fail even though the
// model's intent was correct.
//
// This layer implements the "intercept → match → repair → forward" middleware
// pattern (Awais/CommandCode.ai; ToolCritic failure categories 1-3). At
// dispatch time we consult each tool's JSON Schema (from listTools) and
// coerce arguments whose runtime type contradicts the declared type:
//   - string → boolean   ("true"/"True"/"false"/"yes"/"no"/"1"/"0")
//   - string → number    (numeric strings, per schema integer/number)
//   - string → array     (JSON-parsed when it looks like an array, else wrapped)
//   - string → object    (JSON-parsed when the string is valid JSON)
//   - explicit null on OPTIONAL args is dropped (schema-declared nulls kept)
// Every repair is logged to WIRE_FORMAT_REPAIR_LOG so a per-model failure
// catalog accumulates automatically. Repairs are CONSERVATIVE: anything
// ambiguous (unions including 'string', unparseable values, unknown props)
// passes through untouched.

const WIRE_FORMAT_REPAIR_LOG =
  'E:\\__daniel347x\\__Obsidian\\__Inking into Mind\\--TypingMind\\Projects - All\\Projects - Individual\\TODO\\temp\\wire_format_repair.log';

// clientId -> { schemas: Map<toolName, inputSchema>, at: epochMs }
const TOOL_SCHEMA_CACHE = new Map();
// clientId -> Promise (in-flight dedup for listTools)
const TOOL_SCHEMA_INFLIGHT = new Map();

function logWireFormatRepair(entry) {
  try {
    fs.appendFile(
      WIRE_FORMAT_REPAIR_LOG,
      JSON.stringify({
        timestamp: new Date().toISOString(),
        marker: 'wire_format_repair_v1',
        ...entry,
      }) + '\n',
      () => {},
    );
  } catch (e) {
    console.error('wire format repair log failure:', e.message);
  }
}

async function ensureToolSchemas(clientEntry) {
  if (!clientEntry || !clientEntry.client) return null;
  const clientId = clientEntry.id;
  const cached = TOOL_SCHEMA_CACHE.get(clientId);
  if (cached && cached.schemas && cached.schemas.size > 0) return cached.schemas;
  if (TOOL_SCHEMA_INFLIGHT.has(clientId)) return TOOL_SCHEMA_INFLIGHT.get(clientId);

  const p = (async () => {
    try {
      // NOTE: for stdio clients this goes through the patched listTools
      // wrapper in startClient, so connector-enhanced schemas (edit_file's
      // edits_file / strict_per_edit props) are cached too.
      const result = await clientEntry.client.listTools();
      const schemas = new Map();
      for (const tool of (result && result.tools) || []) {
        if (tool && tool.name) {
          schemas.set(tool.name, tool.inputSchema || tool.parameters || null);
        }
      }
      TOOL_SCHEMA_CACHE.set(clientId, { schemas, at: Date.now() });
      return schemas;
    } catch (e) {
      console.error(
        `ensureToolSchemas: listTools failed for client ${clientId} (non-fatal):`,
        e.message,
      );
      return null;
    } finally {
      TOOL_SCHEMA_INFLIGHT.delete(clientId);
    }
  })();
  TOOL_SCHEMA_INFLIGHT.set(clientId, p);
  return p;
}

const WIRE_BOOL_STRINGS = new Map([
  ['true', true],
  ['yes', true],
  ['1', true],
  ['false', false],
...
function coerceAgainstSchema(value, schema, repairs, argPath) {
  if (!schema || typeof schema !== 'object') return value;

  // anyOf/oneOf without a direct type: try each variant conservatively and
  // take the first that produces a coercion. If any variant legitimately
  // allows the value as-is (e.g. a string union member), nothing changes.
  const variants = schema.anyOf || schema.oneOf;
  if (schema.type === undefined && Array.isArray(variants) && variants.length > 0) {
    for (const sub of variants) {
      const coerced = coerceAgainstSchema(value, sub, repairs, argPath);
  ...
  // If the schema explicitly allows strings, never coerce a string.
  if (typeof value === 'string' && types.includes('string')) return value;

  if (typeof value === 'string') {
    const trimmed = value.trim();

    if (types.includes('boolean')) {
      const b = WIRE_BOOL_STRINGS.get(trimmed.toLowerCase());
      if (b !== undefined) {
        repairs.push({ path: argPath, schemaType: 'boolean', from: value, to: b });
        return b;
      }
    }
    ...
    if (types.includes('integer') || types.includes('number')) {
      if (trimmed !== '' && !isNaN(Number(trimmed))) {
        ...
        if (!integerOnly || Number.isInteger(n)) {
          repairs.push({ path: argPath, schemaType: types.join('|'), from: value, to: n });
          return n;
        }
      }
    }
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            repairs.push({
              path: argPath,
              schemaType: 'array',
              from: clipForLog(value, 120),
              to: clipForLog(JSON.stringify(parsed), 120),
            });
            return parsed;
          }
        } catch (e) {
          /* not valid JSON array text - fall through to wrap */
        }
      }
      repairs.push({
        path: argPath,
    ...
    if (types.includes('object') && trimmed.startsWith('{')) {
      try {
        ...
        /* keep the string if it is not valid JSON */
      }
    }
  }

  // Recurse into plain objects with declared properties
  if (
    value &&
    typeof value === 'object' &&
    !Array.isArray(value) &&
    ...
    for (const [k, v] of Object.entries(value)) {
      ...
      const nv = coerceAgainstSchema(v, sub, repairs, argPath ? `${argPath}.${k}` : k);
  ...
  // Recurse into arrays with a declared items schema
  if (Array.isArray(value) && schema.items) {
    const out = value.map((v, i) =>
      coerceAgainstSchema(v, schema.items, repairs, `${argPath}[${i}]`),
    );
...
function schemaAllowsNull(propSchema) {
  if (!propSchema || typeof propSchema !== 'object') return false;
  if (propSchema.type === 'null') return true;
  if (Array.isArray(propSchema.type) && propSchema.type.includes('null')) return true;
  const variants = propSchema.anyOf || propSchema.oneOf;
  if (Array.isArray(variants)) {
    return variants.some((s) => s && typeof s === 'object' && schemaAllowsNull(s));
  }
  return false;
}

function repairToolArgsAgainstSchema(toolName, toolArgs, schemas) {
  const repairs = [];
  if (!toolArgs || typeof toolArgs !== 'object' || Array.isArray(toolArgs)) {
    return { args: toolArgs, repairs };
  }
  const schema = schemas ? schemas.get(toolName) : null;
  ...
  for (const [key, value] of Object.entries(toolArgs)) {
    ...
    // Repair: drop explicit null on OPTIONAL args (models send null where
    // the schema wants the field omitted). Keep nulls the schema allows.
    if (value === null && !required.includes(key) && !schemaAllowsNull(propSchema)) {
      repairs.push({ path: key, schemaType: 'drop-null-optional', from: null, to: '(removed)' });
      continue;
    }

    if (!propSchema) {
      out[key] = value; // unknown/hallucinated prop: pass through untouched
      continue;
    }
    out[key] = coerceAgainstSchema(value, propSchema, repairs, key);
  }
...
async function dispatchToolCall(clientEntry, toolName, toolArgs) {
  ...
  // Stage -1: schema-driven wire-format arg repair (v18). Runs BEFORE every
  // other stage so downstream strict comparisons (e.g. replaceAll === true)
  // and backend deserializers (e.g. Rust u64) see correctly-typed values.
  try {
    const schemas = await ensureToolSchemas(clientEntry);
    if (schemas) {
      const repaired = repairToolArgsAgainstSchema(
        toolName,
        context.toolArgs,
        schemas,
      );
      if (repaired.repairs.length > 0) {
        context.toolArgs = repaired.args;
        toolArgs = repaired.args;
        logWireFormatRepair({
          toolName,
          clientId: clientEntry.id,
        ...
        console.log(
          `🔧 wire-format repair [${toolName}]: ${repaired.repairs.length} arg(s) coerced (${repaired.repairs
            .map((r) => `${r.path}->${r.schemaType}`)
            .join(', ')})`,
        );
      }
    }
  } catch (e) {
    console.error('wire-format repair stage failed (non-fatal):', e.message);
  }
  ...
  if (context.toolName === 'edit_file') {
    ...
    const candidatePath = context.dryRunOriginalPath || args.path;
    ...
    const reportedPath =
      context.dryRunOriginalPath || cleanedArgs.path || args.path;
    let effectivePath =
      (isDryRunRequested && context.dryRunWorkingPath) ||
      cleanedArgs.path ||
      args.path ||
      reportedPath;
    let originalFileBuffer = null;
    let dryRunWorkingDir = context.dryRunWorkingDir || null;

    if (reportedPath) {
      try {
        const buf = await fs.promises.readFile(reportedPath);
        if (buf) {
          originalFileBuffer = buf;
          let normalizedPath;
          try {
            normalizedPath = path.normalize(reportedPath).toLowerCase();
          } catch (e) {
            normalizedPath = (reportedPath || '').toLowerCase();
          }
          ...
          if (sessionHash && !sessionHashArg) {
            ...
            try {
              ...
              if (existingMeta && existingMeta.original_path) {
                ...
                let resumeNorm;
                try {
                  resumeNorm = path.normalize(reportedPath).toLowerCase();
                } catch (e) {
                  resumeNorm = (reportedPath || '').toLowerCase();
                }
          ...
          await fs.promises.copyFile(reportedPath, origBackupPath);
        }
      } catch (e) {
        console.error(
    ...
    // Deferred-validation dual path (v14): multi-edit batches (>1 edit) run
    // against a private shadow working copy with validation deferred to the
    // end of the batch; legacy per-edit validation is the fallback path.
    // dryRun, single-edit batches, strict_per_edit: true, and unseedable
    // targets (e.g. brand-new files) keep the legacy inline pipeline below.
    if (
      !isDryRunRequested &&
      cleanedArgs.edits.length > 1 &&
      originalFileBuffer &&
      !(args.strict_per_edit === true || args.strictPerEdit === true)
    ) {
      return await runMultiEditOrchestrator(clientEntry, context, {
        cleanedArgs,
        args,
        reportedPath,
        basePath,
    ...
    if (isDryRunRequested && reportedPath) {
      if (context.dryRunWorkingPath) {
        effectivePath = context.dryRunWorkingPath;
      } else {
        try {
          const dryRunClone = await createDryRunWorkingCopy(
            reportedPath,
            originalFileBuffer,
          );
          dryRunWorkingDir = dryRunClone.tempDir;
          effectivePath = dryRunClone.workingPath;
          context.dryRunWorkingPath = dryRunClone.workingPath;
          context.dryRunWorkingDir = dryRunClone.tempDir;
        } catch (e) {
          console.error(
            'edit_file dryRun: failed to prepare temporary working copy:',
            reportedPath,
            e,
          );
          return {
            ...
            content: [
              {
                type: 'text',
                text:
                  `edit_file dryRun: failed to prepare temporary working copy for ${reportedPath}.\n\n` +
                  String(e),
              },
            ],
          };
    ...
    for (let i = 0; i < cleanedArgs.edits.length; i += 1) {
      ...
      const singleContext = {
        ...context,
        useDirectReplaceAll: true,
        displayPath: reportedPath || basePath,
        toolArgs: {
          ...cleanedArgs,
          path: effectivePath,
          edits: [edit],
    ...
    if (isDryRunRequested) {
      ...
      if (
        failedEdits.some((e) => e && e.stage2_validation_failed === true)
      ) {
        summaryLines.push(
          '⚠️ DRY RUN LIMITATION: dryRun validates after EVERY individual edit, so it CANNOT preview batches whose INTERMEDIATE states are invalid but whose FINAL state is valid (edits intentionally composed so they only parse once combined). At least one edit in this dry run failed Stage 2 validation. If that composition was intentional, re-run this same edit_file call WITHOUT dryRun: the live multi-edit pipeline now defers validation to the END of the batch by default and will accept the batch if the final file validates. (Pass strict_per_edit: true on the live call to force legacy per-edit validation.)',
        );
    ...
    await cleanupDryRunWorkingCopy(dryRunWorkingDir);
    return finalResult;
  }
...
async function startClient(clientId, config) {
            ...
            if (schema && schema.type === 'object') {
              ...
              const newProps = {
                ...
                strict_per_edit: {
                  type: 'boolean',
                  description:
                    'Optional escape hatch for the multi-edit pipeline. By default (v14+), batches with more than one edit run with validation DEFERRED to the end of the batch: all edits are applied to a shadow working copy, the final file is validated once, and the batch is accepted wholesale if it validates (legacy per-edit validation is used only as a fallback). Set strict_per_edit: true to force the legacy behavior where every individual edit is validated immediately after it is applied.',
                },
              };
  ...
  // v18: invalidate any cached tool schemas for this client so the
  // wire-format repair layer re-fetches fresh schemas after a (re)start.
  TOOL_SCHEMA_CACHE.delete(clientId);
...
async function start(authToken) {
  ...
  app.use(cors());
  // Raise the JSON request-body limit from the body-parser default (100kb =
  // 102400 bytes) so large tool-call payloads from TypingMind (e.g. big
  // write_file / anchor-capture bodies) are not rejected with
  ...
  try {
    ...
    if (cached && typeof cached === 'object') {
      ...
      // v20 (Manager-Session-036): boot-time Workflowy nodes-cache refresh. A freshly spawned
      // Workflowy daemon warm-starts from the last snapshot on disk, which is stale unless a
      // refresh happened to be the last thing done before it was killed; the widget's Refresh
      // button cannot help before the daemon exists. So once the cached auto-start has brought
      // the shim (and through it the daemon) up, ask it for ONE cache refresh. Fire-and-forget:
      // the connector does not wait on it. Runs only on connector BOOT - never on /start - so it
      // cannot loop. Dan starts the connector by hand; a crash-restart of the daemon by the shim
      // does not come through here. The daemon-side, option-driven version is NEXUS to-do work.
      const wfBoot = clients.get('workflowy');
      if (wfBoot) {
        // v21: local timestamps, elapsed time, epoch + snapshot file (to line up with the daemon
        // console's warm-start line), and the daemon's in-band {success:false} treated as failure.
        const stamp = () => new Date().toLocaleString('sv-SE');
        const t0 = Date.now();
        console.log(`[${stamp()}] [MCP CONNECTOR] Boot-time Workflowy nodes-cache refresh STARTED (system cache.refresh) - do not trust Workflowy reads until the DONE line.`);
        wfBoot.client
          .callTool(
            { name: 'system', arguments: { verb: 'cache.refresh', params: {} } },
            undefined,
            { timeout: 900000 },
          )
          .then((r) => {
            ...
            const text =
              r && Array.isArray(r.content) && r.content[0] && typeof r.content[0].text === 'string'
                ? r.content[0].text
                : '';
            let body = null;
            try {
            ...
            const res = body && typeof body === 'object' && body.detail && typeof body.detail === 'object' ? body.detail : body;
            const pick = (k) => {
              if (res && res[k] !== undefined) return res[k];
              const m = text.match(new RegExp(`"${k}"\\s*:\\s*"?([^",}]+)`));
              return m ? m[1] : undefined;
            };
            const ok = !(r && r.isError) && !(body && body.success === false) && !(res && res.success === false);
            if (!ok) {
              console.warn(`[${stamp()}] ⚠️ [MCP CONNECTOR] Boot-time cache refresh FAILED after ${secs}s - the daemon is still on its warm-start snapshot. Refresh from the widget. Detail: ${text.slice(0, 400)}`);
              return;
            }
            const snap = String(pick('base_snapshot_path') || '').split(/[\\/]/).pop();
            console.log(`[${stamp()}] ✅ [MCP CONNECTOR] Boot-time Workflowy cache refresh DONE in ${secs}s - ${pick('node_count') || '?'} nodes, epoch ${pick('epoch_number') || '?'}${snap ? `, snapshot ${snap}` : ''}. The daemon's in-memory cache is now this epoch.`);
          })
          .catch((e) => {
            const secs = ((Date.now() - t0) / 1000).toFixed(1);
            console.warn(`[${stamp()}] ⚠️ [MCP CONNECTOR] Boot-time cache refresh: no answer after ${secs}s (${e && e.message ? e.message : e}). If this was a timeout, the daemon may still finish it - verify from the widget or system cache.refresh.`);
          });
      }
    }
  } catch (e) {
  ...
  app.post('/start', auth, async (req, res) => {
    try {
      ...
      // v19: cache-drift warning. The cache is maintained by hand, so it can
      // silently fall behind TypingMind's saved config. A server missing from
      // the cache is NOT auto-started at connector boot -- it only starts when
      // TypingMind first POSTs /start (i.e. on the first chat message). That is
      // exactly how the Workflowy daemon stopped booting with the connector
      // (2026-09-22, Manager-Session-036). Say so loudly; never print secrets.
      try {
        const cachedForDrift = loadCachedMcpServers() || {};
        for (const [serverId, config] of Object.entries(mcpServers)) {
          if (!Object.prototype.hasOwnProperty.call(cachedForDrift, serverId)) {
            console.warn(
              `⚠️ [MCP CONNECTOR] Cache drift: '${serverId}' is configured in TypingMind but MISSING from ${MCP_SERVERS_CACHE_PATH} -- it will NOT auto-start when the connector boots. Copy its TypingMind entry into the cache file.`,
            );
          } else if (stringify(cachedForDrift[serverId]) !== stringify(config)) {
            console.warn(
              `⚠️ [MCP CONNECTOR] Cache drift: '${serverId}' differs between TypingMind and the cache file -- it booted from the cached entry and will now be RESTARTED with TypingMind's. Make the cache entry identical to TypingMind's.`,
            );
          }
        }
      } catch (e) {
        console.error('Cache-drift check failed (non-fatal):', e);
      }
