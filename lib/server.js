...
function buildAraxisCommand(leftPath, rightPath) {
  ...
  //   #ARAXIS_SAFE
  //   Start-Process -FilePath 'C:\\...\\Merge.exe' -ArgumentList 'leftPath', 'rightPath'
  //
  // #ARAXIS_SAFE is a marker the safety guard can use to recognize this
  // normalized form. Start-Process is non-blocking (no -Wait), so Araxis
  // launches without blocking the MCP tool call.
  const psCommand =
    "#ARAXIS_SAFE\n" +
